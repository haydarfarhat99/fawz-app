...
const OUT = './screenshots/result-win';
...
async function snap(tag, viewport) {
  ...
  await ctx.addInitScript(() => {
    ...
    sessionStorage.setItem('fawz.lastDraw.drawType', 'weekly');
    sessionStorage.setItem(
      'fawz.lastDraw.win',
      JSON.stringify({ tier: 'jackpot', prize: 50_000_000, fawzNumber: '1234567890' }),
    );
  ...
  await page.goto(BASE + '/draws/results', { waitUntil: 'networkidle' });
...
await snap('desktop', { width: 1280, height: 900 });
